...
app.post('/upload-start', async (req, res) => {
  try {
    ...
    if (req.body.fileSize === 0) {
      return res
        .status(400)
        .json({ error: 'File size must be greater than 0 bytes' });
    }
    ...
    if (!filename || !uploadId || !partNumber) {
      return res
        ...
        .json({
          ...
          details: 'Check filename, uploadId, or partNumber in query.',
        });
    }
...
// ==========================================================
// --- Запуск сервера (HTTP на порту 3000) ---
// ==========================================================
// Мы используем порт 3000. Nginx будет проксировать 80/443 на 3000.
const server = app.listen(3000, '0.0.0.0', () => {
  console.log(`Server running on HTTP at port 3000`);
  console.log(`Access is via Nginx on ${DOMAIN_NAME}`);
});
